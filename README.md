为你把 README.md 补全了！我把整个从 后端部署（Docker Compose） 到 前端集成（Surge 模块） 的全套流程都以优雅、清晰的格式写进去了，别人看了一眼就能跑通。
你可以直接复制下面这段完整的 Markdown 文本去覆盖你的 README.md：
# jovery - 个人代理规则库
...
services:
  secure-monitor:
    image: hk2fs/secure-monitor:v1.0
    container_name: secure-monitor
    restart: always
    network_mode: host       # 使用 host 模式，保证网卡流量和系统指标读取 100% 精确
    environment:
...
2. 点击 “安装新模块”，粘贴以下本仓库的模块原始链接： [https://raw.githubusercontent.com/vplkos/jovery/main/modules/vps_monitor.sgmodule](https://raw.githubusercontent.com/vplkos/jovery/main/modules/vps_monitor.sgmodule)
3. 安装完成后，在模块列表中单击该模块，Surge 将自动弹出交互式配置菜单： • 📥 后端地址：填写你的服务器公网 IP 与端口（例如 http://3.112.233.133:40728）。 • 🔑 鉴权密钥：填写你在 Docker 中配置的 API_KEY。 • 🏷️ 面板标题：自定义你显示在 Surge 首页的服务器名称。 • 🎨 面板图标：可自由更换 SF Symbols 图标（如 party.popper 或 server.rack）。 • ⏱️ 刷新间隔：建议保持默认的 15 秒或 30 秒，兼顾实时性与省电。
